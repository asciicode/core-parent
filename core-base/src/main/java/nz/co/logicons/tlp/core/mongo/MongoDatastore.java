...
public class MongoDatastore
{
  ...
  @Autowired
  private ValidateDocumentOperation validateDocumentOperation;
  ...
  public List<DocumentNode> addAllDocument(List<DocumentNode> documentNodes)
  {
    for (DocumentNode documentNode : documentNodes)
    {
      prepareAndValidateForAddDocument(documentNode);
    }

    for (DocumentNode documentNode : documentNodes)
    {
      upsertDocument(documentNode);
    }

    return documentNodes;
  }

  public DocumentNode addDocument(DocumentNode documentNode)
  {
    prepareAndValidateForAddDocument(documentNode);
    upsertDocument(documentNode);

    // if(runscript)
    // scriptOperation.executeServerScript(ScriptTrigger.AfterSaveServer, documentNode, null, cache, scriptLog, user);

    // datastore.syncDocument(documentNode);

    // scriptLog.setDisplayName(String.format("Add %s %s", documentNode.getSchema().getId(), documentNode.getId()));
    return documentNode;
  }

  private void prepareAndValidateForAddDocument(DocumentNode documentNode)
  {
    Node<?> idNode = documentNode.getChildNode(GenericSerializable.ID_FIELD_NAME);
    if (idNode.getSchema().getType() == ChildSchemaType.autoidschema)
    {
      documentNode.setChildNodeValue(GenericSerializable.ID_FIELD_NAME, generateId());
    }

    if (idNode.getSchema().getType() == ChildSchemaType.sequenceidschema)
    {
      if (idNode.getValue() == null
          || documentNode.getChildNodeValueAsBigDecimal(idNode.getSchema().getId()).intValue() == 0)
      {
        String sequenceid = ((SequenceIdSchema) idNode.getSchema()).getSequenceid();
        BigDecimal sequence = nextSequenceDocument(sequenceid)
            .getChildNodeValueAsBigDecimal(SequenceOperation.NEXT_FIELD);
        documentNode.setChildNodeValue(GenericSerializable.ID_FIELD_NAME, sequence);
      }
    }
    Authentication auth = SecurityContextHolder.getContext().getAuthentication();
    setIdsOnInline(documentNode, documentNode.getId());
    documentNode.getAudit().setCreateFields(auth.getName());
    if (!documentNode.isLogicalDeleted() && !documentNode.getSchema().isSkipvalidation())
    {
      validateDocumentOperation.validate(documentNode);
    }

    if (exists(documentNode.getSchema().getId(), documentNode.getId()))
    {
      DocumentNode LogicalDeletedDocumentNode = getDocument(documentNode.getSchema().getId(), documentNode.getId(),
          false);
      String errMsg = "";
      if (LogicalDeletedDocumentNode.isLogicalDeleted())
      {
        errMsg = "Duplicate of deleted item";
      }
      else
      {
        errMsg = "Duplicate";
      ...
      jsonErrors.add(new SimpleJsonError(GenericSerializable.ID_FIELD_NAME, errMsg, ""));
      throw new ValidationException(jsonErrors);
    }
  }

  public boolean exists(String schemaid, String documentid)
  {
    ...
    if (documentSchema != null)
    {
      numericid = documentSchema.isDocumentIdNumeric();
    }
    return get(schemaid, documentid, numericid) != null;
  }
  ...
  /**
   * Ensure that every inline document (including inlines in lists / irrespective of depth) have a childid and parentid.
   * 
   * @param documentNode .
   * @param parentid .
   */
  private void setIdsOnInline(DocumentNode documentNode, String parentid)
  {
    if (documentNode.getValue() == null)
    {
      return;
    }

    // get the schema
    DocumentSchema documentSchema = documentNode.getSchema();

    if (documentSchema.isInline())
    {
      documentNode.setChildNodeValue(GenericSerializable.PARENT_ID_FIELD_NAME, parentid);

      if (StringUtils.isBlank(documentNode.getChildNodeValueAsString(GenericSerializable.CHILD_ID_FIELD_NAME)))
      {
        documentNode.setChildNodeValue(GenericSerializable.CHILD_ID_FIELD_NAME, generateId());
      }

      parentid = documentNode.getChildNodeValueAsString(GenericSerializable.CHILD_ID_FIELD_NAME);
    }

    for (ChildSchema<?> childSchema : documentSchema.getChildrenMap().values())
    {
      if (childSchema.getType() == ChildSchemaType.inlineschema)
      {
        DocumentNode inlineDocumentNode = (DocumentNode) documentNode.getChildNode(childSchema.getId());
        setIdsOnInline(inlineDocumentNode, parentid);
      }
      else if (childSchema.getType() == ChildSchemaType.listinlineschema)
      {
        List<DocumentNode> inlineDocumentNodes = documentNode.getChildNodeValueAsDocumentNodeList(childSchema.getId(),
            this);
        for (DocumentNode inlineDocumentNode : inlineDocumentNodes)
        {
          setIdsOnInline(inlineDocumentNode, parentid);
        }
      }
